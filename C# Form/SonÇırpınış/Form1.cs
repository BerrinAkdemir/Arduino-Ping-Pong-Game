...
namespace SonÇırpınış
{
    public partial class Form1 : Form
    {
        ...
        private void seri_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            try
            {
                ...
                if (values.Length == 6)
                {
                    ...
                    {
                        // Joystick 1, Raket 1 için AŞAĞI hareket kodu
                        raketbir.Top += 5;
                    }
                    ...
                    {
                        // Joystick 2, Raket 2 için AŞAĞI hareket kodu
                        raketiki.Top += 5;
                    }
                }
            }
        ...
        {
            //Topun hareketini belirlemek için topun sol ve üst pozisyonlarına belirli hızları ekliyor. Bu, topun belirli bir hızda hareket etmesini sağlar.
            ball.Left += ballhizX;
            ball.Top += ballhizY;


            //Duvara çarptığını kontrol et. 
            if (ball.Top <= panel1.Top || ball.Bottom >= panel1.Bottom)
                ballhizY *= -1;


            ////Raketlerle çarpışma kontrolü yapılıyor. 
        ...
        int[] raketY = new int[2]; // İki raket için Y pozisyonlarını saklayacak dizi
        private void MoveRackets()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                // Raket sınırlarını kontrol etme
                if (i == 0)
                {
                    if (normalizedY + raketbir.Height > panel1.Height - 10) // Varsayılan olarak raket1'in yüksekliği kullanılıyor
                        normalizedY = panel1.Height - raketbir.Height - 50;
                    raketbir.Top = normalizedY;
                }
                else if (i == 1)
                {
                    if (normalizedY > panel1.Height - raketiki.Height) // Varsayılan olarak raket2'in yüksekliği kullanılıyor
